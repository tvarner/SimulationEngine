...
export default function buildTransformControls(THREE) {
	...
	THREE.TransformGizmoRotate = function () {
		...
		const CircleGeometry = function ( radius, facing, arc ) {
			...
			const vertices = [];
			const _arc = arc ? arc : 1;

			for ( let i = 0; i <= 64 * _arc; ++ i ) {
	...
	THREE.TransformControls = function ( camera, domElem ) {
		// TODO: Make non-uniform scale and rotate play nice in hierarchies
		// TODO: ADD RXYZ contol

		THREE.Object3D.call( this );

		const domElement = ( domElem !== undefined ) ? domElem : document;
