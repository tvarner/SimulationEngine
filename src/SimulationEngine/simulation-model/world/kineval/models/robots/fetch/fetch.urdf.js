...
robot.eventEmitter = new EventEmitter2();
...
// total_files = robot.fetch_dae_files.length;
// files_loaded = 0;


robot.assignFetchModel = function assignFetchModel(filename,index) {
	console.info("assignFetchModel : "+filename+" - "+index); 
	
	if (!robot.view.colladaLoader) { throw new Error("ColladaLoader must be added to robot"); }
	if (!robot.eventEmitter) { throw new Error("eventEmitter (EventEmitter2) must be added to robot"); }
	// const collada_loader = new THREE.ColladaLoader();

	// const val =
	robot.view.colladaLoader.load(filename, 
			function ( collada ) {
			robot.links_geom[index] = collada.scene;

			// dispatch collada_loaded event once all files loaded
			if (Object.keys(robot.links_geom).length === robot.fetch_dae_files.length) {
				robot.eventEmitter.emit('collada_loaded');
			}
		},
		function (xhr) {
			console.info(filename+" - "+index+": "+(xhr.loaded / xhr.total * 100) + '% loaded' );
		}
	);
};

robot.retrieveCollada = function() { 
	for (let i=0;i<robot.fetch_dae_files.length;i++) {
		const geom_index = robot.fetch_dae_files[i].split('_adjusted');
		...
		// assignFetchModel('./temp_fetch/'+robot.fetch_dae_files[i],geom_index[0]);
		// assignFetchModel('./temp_fetch/'+robot.fetch_dae_files[i],i);
		// console.info('assign '+'./temp_fetch/'+robot.fetch_dae_files[i]);
	}
};
