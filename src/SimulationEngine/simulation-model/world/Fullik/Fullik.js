...
Fullik.getSignedAngleBetweenDegs = function( referenceVector, otherVector, normalVector ) { 
	const unsignedAngle = Fullik.getAngleBetweenDegs( referenceVector, otherVector );
	const sign = Fullik.sign( Fullik.dotProduct( Fullik.crossProduct( referenceVector, otherVector ), normalVector ) );        
...
Fullik.rotateAboutAxisRads = function( v, rad, rotationAxis ) {
	const rotationMatrix = new Fullik.M3();
...
Fullik.getAngleLimitedUnitVectorDegs = function( vecToLimit, vecBaseline, angleLimitDegs ) {
	...
	const angleBetweenVectorsDegs = Fullik.getAngleBetweenDegs( vecBaseline, vecToLimit );
	...
	if ( angleBetweenVectorsDegs > angleLimitDegs ) {           
		...
		const correctionAxis = Fullik.crossProduct( vecBaseline.normalised(), vecToLimit.normalised() ).normalize();
...
Fullik.M3.prototype.isOrthogonal = function() {
	const xCrossYDot = Fullik.dotProduct( this.getXBasis(), this.getYBasis() );
	const xCrossZDot = Fullik.dotProduct( this.getXBasis(), this.getZBasis() );
	const yCrossZDot = Fullik.dotProduct( this.getYBasis(), this.getZBasis() );
...
Fullik.M3.prototype.rotateRads = function( rotationAxis, rad ) {
	...
	const t00 = te[0] * f00 + te[1] * f01 + te[2] * f02;
	const t01 = te[3] * f00 + te[4] * f01 + te[5] * f02;
	const t02 = te[6] * f00 + te[7] * f01 + te[8] * f02;

	const t10 = te[0] * f10 + te[1] * f11 + te[2] * f12;
	const t11 = te[3] * f10 + te[4] * f11 + te[5] * f12;
	const t12 = te[6] * f10 + te[7] * f11 + te[8] * f12;

	// Construct and return rotation matrix
	const tx = dest.elements;
...
Fullik.Bone.prototype = {
	...
	clone:function() {
		const b = new Fullik.Bone( this.mStartLocation, this.mEndLocation );
	...
	setBallJointConstraintDegs:function( angleDegs ) {
		let _angleDegs = angleDegs;

		if (_angleDegs < 0 ) _angleDegs = 0;
		if (_angleDegs > 180 ) _angleDegs = 180;
		this.mJoint.setBallJointConstraintDegs( _angleDegs );
	},
...
Fullik.Chain.prototype = {
	...
	addConsecutiveBone : function( directionUV, length ) {
		...
		if (this.mNumBones > 0) {               
			...
			const prevBoneEnd = this.bones[this.mNumBones-1].getEndLocation();// .clone();
	...
	addConsecutiveHingedBone: function( directionUV, length, type, hingeRotationAxis, clockwiseDegs, anticlockwiseDegs, hingeReferenceAxis ) {
		...
		const _directionUV = directionUV.normalised();
		const _hingeRotationAxis = hingeRotationAxis.normalised();
			
		// Get the end location of the last bone, which will be used as the start location of the new bone
		const prevBoneEnd = this.bones[this.mNumBones-1].getEndLocation();// .clone();
			
		// Create a bone
		const bone = new Fullik.Bone( prevBoneEnd, undefined, _directionUV, length, this.color );

		const _type = type || 'global';
		
		// ...then create and set up a joint which we'll apply to that bone.
		const joint = new Fullik.Joint();
		switch (_type) {
		case 'global':
			joint.setAsGlobalHinge( _hingeRotationAxis, clockwiseDegs, anticlockwiseDegs, hingeReferenceAxis );
			break;
		case 'local':
			joint.setAsLocalHinge( _hingeRotationAxis, clockwiseDegs, anticlockwiseDegs, hingeReferenceAxis );
			break;
		}
	...
	addConsecutiveRotorConstrainedBone:function( boneDirectionUV, length, constraintAngleDegs ) {
		...
		const _boneDirectionUV = boneDirectionUV.normalised();
		const bone = new Fullik.Bone( this.bones[ this.mNumBones-1 ].getEndLocation(), undefined , _boneDirectionUV, length );
	...
	getLiveChainLength:function() {
		let lng = 0;        
		for (let i = 0; i < this.mNumBones; i++) {  
	...
	setRotorBaseboneConstraint : function( type, constraintAxis, angleDegs ) {
		...
		this.mBaseboneConstraintType = _type === 'global' ? Fullik.BB_GLOBAL_ROTOR : Fullik.BB_LOCAL_ROTOR;
		this.mBaseboneConstraintUV = constraintAxis.normalised();
		this.mBaseboneRelativeConstraintUV.copy( this.mBaseboneConstraintUV );
		this.getBone(0).getJoint().setAsBallJoint( _angleDegs );
	...
	setHingeBaseboneConstraint : function( type, hingeRotationAxis, cwConstraintDegs, acwConstraintDegs, hingeReferenceAxis ) {
		...
		// Set the constraint type, axis and angle
		this.mBaseboneConstraintType = _type === 'global' ? Fullik.BB_GLOBAL_HINGE : Fullik.BB_LOCAL_HINGE;
		this.mBaseboneConstraintUV.copy( hingeRotationAxis.normalised() );
		
		// var hinge = this.getBone(0).getJoint();//new Fullik.Joint();
		
		if ( _type === 'global' ) this.getBone(0).getJoint().setHinge( Fullik.J_GLOBAL_HINGE, hingeRotationAxis, cwConstraintDegs, acwConstraintDegs, hingeReferenceAxis );
	...
	updateTarget : function( t ) {
		const newTarget = new Fullik.V3( t.x, t.y, t.z );// .copy(t);// ( newTarget.x, newTarget.y, newTarget.z );
	...
	solveIK : function( target ) {
		...
		while( i-- ) {
			...
			if ( i !== this.mNumBones - 1 ) {
				...
				const outerBoneOuterToInnerUV = this.bones[ i+1 ].getDirectionUV().negated();
				...
				if ( jointType === Fullik.J_BALL ) { 
					...
					const angleBetweenDegs = Fullik.getAngleBetweenDegs( outerBoneOuterToInnerUV, boneOuterToInnerUV );
					const constraintAngleDegs = joint.getBallJointConstraintDegs();
					...
					let m; // M3
					let relativeHingeRotationAxis; // V3
					if ( i > 0 ) {
				...
				const newStartLocation = bone.getEndLocation().plus( boneOuterToInnerUV.times( lng ) );
				...
				case Fullik.J_LOCAL_HINGE: {
					...
					const m = Fullik.createRotationMatrix( this.bones[i-1].getDirectionUV() );
					
					// ...and transform the hinge rotation axis into the previous bones frame of reference.
					const relativeHingeRotationAxis = m.timesV3( joint.getHingeRotationAxis() ).normalize();
				...
				const newStartLocation = target.plus( boneOuterToInnerUV.times( lng ) );
		...
		for ( i = 0; i < this.mNumBones; i++ ) {
			...
			if ( i !== 0 ) {
				...
				if ( jointType === Fullik.J_BALL ) {                   
					const angleBetweenDegs = Fullik.getAngleBetweenDegs( prevBoneInnerToOuterUV, boneInnerToOuterUV );
					const constraintAngleDegs = joint.getBallJointConstraintDegs(); 
					...
					const hingeRotationAxis = joint.getHingeRotationAxis();
					...
					const cwConstraintDegs = -joint.getHingeClockwiseConstraintDegs();
					const acwConstraintDegs = joint.getHingeAnticlockwiseConstraintDegs();
					...
					if ( !( Fullik.nearEquals( cwConstraintDegs, -Fullik.MAX_ANGLE_DEGS, Fullik.PRECISION ) ) && !( Fullik.nearEquals( acwConstraintDegs, Fullik.MAX_ANGLE_DEGS, Fullik.PRECISION ) ) ) {
						const hingeReferenceAxis = joint.getHingeReferenceAxis();
						...
						const signedAngleDegs = Fullik.getSignedAngleBetweenDegs( hingeReferenceAxis, boneInnerToOuterUV, hingeRotationAxis );
					...
					const m = Fullik.createRotationMatrix( prevBoneInnerToOuterUV );
					
					// Transform the hinge rotation axis into the previous bone's frame of reference
					const relativeHingeRotationAxis = m.timesV3( hingeRotationAxis ).normalize();
					...
					if ( !( Fullik.nearEquals( cwConstraintDegs, -Fullik.MAX_ANGLE_DEGS, Fullik.PRECISION ) ) && !( Fullik.nearEquals( acwConstraintDegs, Fullik.MAX_ANGLE_DEGS, Fullik.PRECISION ) ) ) {
						...
						const relativeHingeReferenceAxis = m.timesV3( joint.getHingeReferenceAxis() ).normalize();
						
						// Get the signed angle (about the hinge rotation axis) between the hinge reference axis and the hinge-rotation aligned bone UV
						// Note: ACW rotation is positive, CW rotation is negative.
						const signedAngleDegs = Fullik.getSignedAngleBetweenDegs( relativeHingeReferenceAxis, boneInnerToOuterUV, relativeHingeRotationAxis );
				...
				// so we can set the new inner joint location to be the end joint location of this bone plus the
				// outer-to-inner direction unit vector multiplied by the length of the bone.
				const newEndLocation = bone.getStartLocation().plus( boneInnerToOuterUV.times( lng ) );
				...
				if ( this.mBaseboneConstraintType === Fullik.BB_NONE ) {
					...
					const newEndLocation = bone.getStartLocation().plus( bone.getDirectionUV().times( lng ) );
					bone.setEndLocation( newEndLocation );    
					...
					if ( this.mBaseboneConstraintType === Fullik.BB_GLOBAL_ROTOR ) {   
						...
						const angleBetweenDegs = Fullik.getAngleBetweenDegs( this.mBaseboneConstraintUV, boneInnerToOuterUV );
						const constraintAngleDegs = bone.getBallJointConstraintDegs(); 
						...
						const angleBetweenDegs = Fullik.getAngleBetweenDegs( this.mBaseboneRelativeConstraintUV, boneInnerToOuterUV);
						const constraintAngleDegs = bone.getBallJointConstraintDegs();
						if ( angleBetweenDegs > constraintAngleDegs ) {
						...
						joint = bone.getJoint();
						const hingeRotationAxis = joint.getHingeRotationAxis();
						const cwConstraintDegs = -joint.getHingeClockwiseConstraintDegs(); // Clockwise rotation is negative!
						const acwConstraintDegs = joint.getHingeAnticlockwiseConstraintDegs();
						
						// Get the inner-to-outer direction of this bone and project it onto the global hinge rotation axis
						let boneInnerToOuterUV = bone.getDirectionUV().projectOnPlane( hingeRotationAxis ).normalize();
						...
						if ( !( Fullik.nearEquals( cwConstraintDegs, Fullik.MAX_ANGLE_DEGS, Fullik.PRECISION_DEG ) ) && !( Fullik.nearEquals( acwConstraintDegs, Fullik.MAX_ANGLE_DEGS, Fullik.PRECISION_DEG ) ) ) {
							// Grab the hinge reference axis and calculate the current signed angle between it and our bone direction (about the hinge
							// rotation axis). Note: ACW rotation is positive, CW rotation is negative.
							const hingeReferenceAxis = this.mBaseboneRelativeReferenceConstraintUV; // joint.getHingeReferenceAxis();
							const signedAngleDegs = Fullik.getSignedAngleBetweenDegs( hingeReferenceAxis, boneInnerToOuterUV, hingeRotationAxis );
...
Fullik.Structure.prototype = {
	...
	update:function() {
		...
		// let i =  this.mNumChains;

		// while(i--) {

		for(let i = 0; i < this.mNumChains ; i++) {
			c = this.chains[i];
			m = this.meshChains[i];
			t = this.targets[i];
			...
			if (connectedChainNumber === -1) c.updateTarget( t );
				...
				case Fullik.BB_LOCAL_HINGE: {
					const connectionBoneMatrix = Fullik.createRotationMatrix( hostBone.getDirectionUV() );
					...
					const relativeBaseboneConstraintUV = connectionBoneMatrix.timesV3( c.getBaseboneConstraintUV() ).normalize();
	...
	connectChain : function( newChain, existingChainNumber, existingBoneNumber, boneConnectionPoint, target, meshBone, color ) {
		...
		const relativeChain = newChain.clone();// new Fullik.Chain( newChain );
		...
		const connectionPoint = boneConnectionPoint || this.getChain( existingChainNumber ).getBone( existingBoneNumber ).getBoneConnectionPoint();
		...
		for ( let i = 0; i < relativeChain.getNumBones(); i++ ) {
			const origStart = relativeChain.getBone(i).getStartLocation();
			const origEnd = relativeChain.getBone(i).getEndLocation();
	...
	addBoneMesh:function( bone ) {
		...
		const a1 = bone.getJoint().mHingeClockwiseConstraintDegs * Fullik.torad;
		const a2 = bone.getJoint().mHingeAnticlockwiseConstraintDegs * Fullik.torad;
		...
		const type = bone.getJoint().type;
		switch(type) {
		case Fullik.J_BALL : {
			m2.color.setHex(0xFF6600);
			const angle = bone.getJoint().mRotorConstraintDegs;
			if(angle === 180) break;
			const s = size/4;
