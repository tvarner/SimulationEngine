...
Other objects can extend this class (i.e. Vehicle).
*/
...
export default class MobileAgent { 
		...
		this.activeActionPointQueue = !this.reverse ?
			...
			new PriorityQueue({
				comparator: function comparator(actionPoint1, actionPoint2) {
					const actionPointCounter1 = _.isFunction(actionPoint1.counter) ? actionPoint1.counter(actionPoint1.traversalGraph) : actionPoint1.counter;
					const actionPointCounter2 = _.isFunction(actionPoint2.counter) ? actionPoint2.counter(actionPoint2.traversalGraph) : actionPoint2.counter;                            
	...
	buildBody() {
		...
		const bodyMesh = new THREE.Mesh(this.geometry, this.material);
	...
	buildInternalBody() {
		...
		const mesh = new THREE.Mesh(new THREE.CylinderGeometry( 0, 1, 5, 12, 1, false));
		arrowGeometry.merge( mesh.geometry, mesh.matrix );

		const arrowMesh = new THREE.Mesh(arrowGeometry, new THREE.MeshBasicMaterial({ color: ColorModel.agentColors.mobileAgentColor, wireframe: true }));

		const arrow = new THREE.Object3D();
		arrow.add(arrowMesh);

		const headerArrow = new THREE.Object3D();
	...
	updateMobileAgentPosition(t) {
		...
		if (this.inTransition === true) {
			if (this.counter < 1 && this.counter >= 0) {
				...
				const pos = this.lane.tube.parameters.path.getPointAt(this.counter);

				// get world coordinate of pos relative to cluster
				const clusterPos = this.lane.path.cluster.body.position;
				...
				const dir = this.lane.tube.parameters.path.getTangentAt(this.counter);

				// interpolation
				const segments = this.lane.tube.tangents.length;
				let pickt;
	...
	*/
	getCounterDelta(t, reverse) {
		const l = this.lane.curve.getLength();
		const _l = this.traversalGraph.lengthModel.getSystemLength(l);

		// calculate delta d
		const _d = (this.v * t) + (0.5 * this.a * (t * t));
	...
	checkActiveActionPoint() {
		...
		if (this.activeActionPoint) {
			const activeActionPointCounter = _.isFunction(this.activeActionPoint.counter) ? this.activeActionPoint.counter(this.traversalGraph) : this.activeActionPoint.counter;
			...
			if (this.activeActionPointQueue) { 
				...
				if (!this.reverse) {
					_removeFn = function(actionPoint) { 
						const actionPointCounter = _.isFunction(actionPoint.counter) ? actionPoint.counter(this.traversalGraph) : actionPoint.counter;
						...
						if (actionPoint.laneId === this.lane.id) {
							if (actionPointCounter >= this.counter) {
								...
								const _removeCompletionCheckFn = function(actionPoint) { 
				...
				const _activatedActionPoints = _.remove(this.registeredActionPoints, _removeFn.bind(this));
				// add activated actionPoints to active actionPoint queue
				const _queueActivedActionPointsFn = function(actionPoint) {
