...
export default class CubicLatticeCluster extends Cluster {
		...
		for(let x = 0; x < xNodes; x++) {
			...
			for(let y = 0; y < yNodes; y++) {
				...
				for(let z = 0; z < zNodes; z++) {
					...
					const node = new Node("general", 0.5, _position, x, y, z);
	...
	buildFlower(cx, cy, cz, nodeRadius) {
		...
		const startNode = this._getNodeByIndex(cx, cy, cz);

		const endNode1 = this._getNodeByIndex(cx + r, cy + r, cz);
		const endNode2 = this._getNodeByIndex(cx, cy + r, cz + r);
		const endNode3 = this._getNodeByIndex(cx - r, cy + r, cz);
		const endNode4 = this._getNodeByIndex(cx, cy + r, cz - r);

		const endNode5 = this._getNodeByIndex(cx, cy - r, cz - r);
		const endNode6 = this._getNodeByIndex(cx - r, cy - r, cz);
		const endNode7 = this._getNodeByIndex(cx, cy - r, cz + r);
		const endNode8 = this._getNodeByIndex(cx + r, cy - r, cz);

		const endNode9 = this._getNodeByIndex(cx + r, cy, cz - r);
		const endNode10 = this._getNodeByIndex(cx + r, cy, cz + r);
		const endNode11 = this._getNodeByIndex(cx - r, cy, cz + r);
		const endNode12 = this._getNodeByIndex(cx - r, cy, cz - r);
