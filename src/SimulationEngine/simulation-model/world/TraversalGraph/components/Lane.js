...
export default class Lane { 
	...
	getNodeEntrancePosition() {
		...
		const nodeEntrancePosition = new THREE.Vector3();
	...
	removeMobileAgent(mobileAgentId) {
		_.remove(this.mobileAgents, function(mobileAgent) {
			return (mobileAgent.id === mobileAgentId);
		});
	}
	...
	buildActionPoints(actionPoints) { 
		for (let i = 0; i < actionPoints.length; i++) {
			const actionPoint = actionPoints[i];
			...
			const actionPosition = this.curve.getPointAt(_counter);

			// create [action] node at action position
			const actionNode = new Node("action", 1, actionPosition);
